				...
				if (isCount == 0) {
					...
					if (noneload == undefined) {
						if ((newVideo != undefined) && GALAXY.oldIos && GALAXY.isMobile) {
							...
							$('video').each(function () {
								if($(this).closest('figure').attr('data-muted') !== 'false') {
									$(this)[0].play();
								}
							});
						}
				...
				if ((newVideo != undefined) && GALAXY.oldIos && GALAXY.isMobile) {
					...
					if ((noneplay != 'none') && ($(this).find('video').length > 0)) {
						if (noneload == undefined) {
							...
							// 19.04.30 Issue - Video not playing when be visible area before video load
							$(this).find('video').on('canplay', function(){
								if(!justOne){
									$figure.find('.hide-bg').css('opacity', 0);
									$(this)[0].play();
								}
							});
						}
		...
		function changSrc() {
			if (changeVideo != undefined) {
				...
				var changeSize = function (sizeInner) {
					...
					$video.html('');
					$video.html(createSources).removeClass('ready playend'); //20.01.20 add removeClass
...
GALAXY.isGalaxy && $(window).load(function() {
	setTimeout(function() {
		GALAXY.initialize();
	}, 0);
});
